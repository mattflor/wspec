...
parameters = dict(s=s, threshold=threshold, hms_prec=hms_prec)           # dictionary for storing simulation
...
- system is checked for stability of the HMS pattern
"""
snum = 'mcrit_6'
...
migration_rate = m = 0.1           # symmetric migration
mig_decimals = 3
...
while mig_diff > mig_prec:
    ...
    R_ = IT_ * IA_ * IB_ * IC_ * ID_ * IE_ * IF_ * \
         HMS_AB_ * \
         HMS_AC_ * HMS_BC_ * \
    ...
    startfreqs[0,1,0,1,0,1,0,0] = 1.   # pop1-A1-B0-C1-D0-E1-F0-T1
    startfreqs[1,0,1,0,1,0,1,1] = 1.   # pop2-A0-B1-C0-D1-E0-F1-T2
    starttime = time.time()

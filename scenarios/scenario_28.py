...
parameters = dict(m=m, lCI=l, f=f, t=t, e=e, s=s, pt=pt, intro=intro, threshold=threshold)           # dictionary for storing simulation
...
#! Reproduction
#!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#! Cytoplasmic incompatibility, suppression by A1-C1
#!----------------------------------------------------------------------
CI = core.ReproductionWeight(name='cytoplasmic incompatibility (suppressed)', \
                             axes=['male_Alocus', 'male_Clocus', 'male_cytotype', 'offspring_cytotype'], \
                             config=config, \
                             unstack_levels=[-1], \
                             lCI=ci_level, \
                             e=mod_penetrance
...
ary = np.array( [[[[1,          1], \
                 ...
                 [[[1,          1], \
                   [1-ci,       1]], \
                  [[1,          1], \
                   [1-(1-e)*ci, 1]]]], float )       # mA1-mC1-mW oU    modified CI case
CI.set( ary )
CI_ = CI.extended()
print CI
...
- in males, A1-C1 acts as a suppressor of Wolbachia's mod function

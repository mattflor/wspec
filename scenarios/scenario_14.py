...
#! Loci and alleles
#!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
LOCI = ['population', 'backA', 'backB', 'recognition', 'trait', 'preference']
...
rejection_probability_species1 = pr_s1 = 0.9
rejection_probability_species2 = pr_s2 = 0.7
...
parameters = dict(m=m, s=s, pt=pt, pr_s1=pr_s1, pr_s2=pr_s2, pr_t3=pr_t3, pr_t4=pr_t4, intro=intro, threshold=threshold)           # dictionary for storing simulation
...
#! Trait preference
#!----------------------------------------------------------------------
trait_preferences = {'P1': {'all pops': ('T2', pr_t3)}, \
                     'P2': {'all pops': ('T3', pr_t4)}
                    }
...
startfreqs[1,0,0,0,1,0] = 1.                   # pop2-A1-B1-S1-T2-P0-U
startfreqs[2,1,1,1,2,0] = 1.                   # pop3-A1-B1-S1-T3-P0-U
startfreqs[3,1,1,1,3,0] = 1.                   # pop4-A2-B2-S2-T4-P0-W
starttime = time.time()

...
#! Loci and alleles
#!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
LOCI = ['population', 'backA', 'backB', 'recognition', 'trait', 'cytotype']
...
parameters = dict(m=m, s=s, lCI=l, f=f, t=t, pt=pt, pr_s1=pr_s1, pr_s2=pr_s2, intro=intro, threshold=threshold)           # dictionary for storing simulation
...
weights['dynamic_reproduction'] = [(SR,['backA', 'backB'])]
print SR
...
R_ = CI_ * F_ * T_ * TI_ * AI_ * BI_ * SI_ * HMS_
...
- allele S0 (no female preference for background loci A and B) is introduced at equilibrium in all pop.s
...
startfreqs[0,0,0,1,0,0] = 1.                   # pop1-A1-B1-S1-T1-P0-U
startfreqs[1,0,0,1,1,0] = 1.                   # pop2-A1-B1-S1-T2-P0-U
startfreqs[2,0,0,1,2,0] = 1.                   # pop3-A1-B1-S1-T3-P0-U
startfreqs[3,1,1,2,3,1] = 1.                   # pop4-A2-B2-S2-T4-P0-W
starttime = time.time()
...
#! Introduction of species recognition allele S0
#!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
intro_allele = 'S0'
...
metapop.introduce_allele('pop3', intro_allele, intro_freq=intro, advance_generation_count=False)
